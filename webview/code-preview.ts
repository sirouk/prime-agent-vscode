/**
 * Cell previews, ported verbatim from prime-agent
 * (packages/coding-agent/src/core/tools/{code-preview,ipython-cell-code}.ts).
 *
 * Why vendored: the RPC never sends a preview — ToolExecutionStartEvent carries
 * only {toolCallId, toolName, args} — so the collapsed tool card has to derive
 * the "what actually ran" line itself. Keeping prime-agent's own scorer means
 * the extension and the CLI describe the same cell the same way instead of the
 * extension inventing a second, worse heuristic.
 *
 * Pure: no imports, no DOM, no runtime deps. Keep it a straight port so it can
...
/** `%%bash` cell magic: the shell script is everything after the magic line. */
