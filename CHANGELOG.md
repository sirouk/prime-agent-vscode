...
- **The Processes panel now lists Prime Agent `background_task` jobs.** The skill writes receipts under the session's `background-tasks/` directory and wakes the owning session when a job ends, but the panel had no path to that folder — so a 30-second sleep the agent started was visible only as a later turn, never as a running row. The panel now reads those receipts for the session on screen (a child's jobs stay on the child until you browse into it), shows a real exit code when one finishes, and tails the skill's own stdout/stderr on click.
